/* test-suitability-gate.mjs — contract-conformance smoke test for SuitabilityGate.
   Dependency-free: run with `node test-suitability-gate.mjs` (adjust the import paths to your project). */
...
const id = "SuitabilityGate";
const c = core.getComponent(id);
assert.ok(!c.error, 'component resolves');

// 1. every declared token resolves in the system
for (const t of ["surface","border","text1","red","space.4"]) assert.ok(core.tokenKnown(t), 'token resolves: ' + t);
...
assert.deepStrictEqual(c.regulatory || [], ["FINRA Rule 2111 (suitability)","SEC Reg BI"], 'regulatory anchors intact');
