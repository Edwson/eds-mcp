/* test-reg-citation.mjs — contract-conformance smoke test for RegCitation.
   Dependency-free: run with `node test-reg-citation.mjs` (adjust the import paths to your project). */
...
const id = "RegCitation";
const c = core.getComponent(id);
assert.ok(!c.error, 'component resolves');

// 1. every declared token resolves in the system
for (const t of ["text3","type.xs"]) assert.ok(core.tokenKnown(t), 'token resolves: ' + t);
// 2. declared states are a subset of the canonical states
for (const s of []) assert.ok(core.CANON_STATES.includes(s), 'canonical state: ' + s);
// 3. regulated component carries its anchor(s)
assert.deepStrictEqual(c.regulatory || [], ["FinCEN CDD","FATF Rec 10/12"], 'regulatory anchors intact');
