/* test-sanctions-screen.mjs — contract-conformance smoke test for SanctionsScreen.
   Dependency-free: run with `node test-sanctions-screen.mjs` (adjust the import paths to your project). */
...
const id = "SanctionsScreen";
const c = core.getComponent(id);
assert.ok(!c.error, 'component resolves');

// 1. every declared token resolves in the system
for (const t of ["surface","border","red","gold","text2"]) assert.ok(core.tokenKnown(t), 'token resolves: ' + t);
...
assert.deepStrictEqual(c.regulatory || [], ["OFAC 31 CFR Part 501","FATF Rec 6 (targeted financial sanctions)"], 'regulatory anchors intact');
