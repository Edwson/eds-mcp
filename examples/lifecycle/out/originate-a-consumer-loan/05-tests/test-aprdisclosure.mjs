/* test-aprdisclosure.mjs — contract-conformance smoke test for APRDisclosure.
   Dependency-free: run with `node test-aprdisclosure.mjs` (adjust the import paths to your project). */
...
const id = "APRDisclosure";
const c = core.getComponent(id);
assert.ok(!c.error, 'component resolves');

// 1. every declared token resolves in the system
for (const t of ["text1","text2","gold","surface","border","radius.md","space.4"]) assert.ok(core.tokenKnown(t), 'token resolves: ' + t);
// 2. declared states are a subset of the canonical states
for (const s of ["loading","error"]) assert.ok(core.CANON_STATES.includes(s), 'canonical state: ' + s);
// 3. regulated component carries its anchor(s)
assert.deepStrictEqual(c.regulatory || [], ["TILA / Reg Z 12 CFR 1026.18 (APR + finance charge disclosure)","UK FCA CONC 4.2 (APR prominence)"], 'regulatory anchors intact');
