/* test-affordability-check.mjs — contract-conformance smoke test for AffordabilityCheck.
   Dependency-free: run with `node test-affordability-check.mjs` (adjust the import paths to your project). */
...
const id = "AffordabilityCheck";
const c = core.getComponent(id);
assert.ok(!c.error, 'component resolves');

// 1. every declared token resolves in the system
for (const t of ["green","gold","red","text1","surface","radius.md","space.4"]) assert.ok(core.tokenKnown(t), 'token resolves: ' + t);
...
assert.deepStrictEqual(c.regulatory || [], ["UK FCA CONC 5 (creditworthiness & affordability)","ECOA / Reg B (consistent, non-discriminatory assessment)"], 'regulatory anchors intact');
