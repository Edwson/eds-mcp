/* test-loan-origination-stepper.mjs — contract-conformance smoke test for LoanOriginationStepper.
   Dependency-free: run with `node test-loan-origination-stepper.mjs` (adjust the import paths to your project). */
...
const id = "LoanOriginationStepper";
const c = core.getComponent(id);
assert.ok(!c.error, 'component resolves');

// 1. every declared token resolves in the system
for (const t of ["accent2","green","red","text1","border","radius.md","space.4"]) assert.ok(core.tokenKnown(t), 'token resolves: ' + t);
...
assert.deepStrictEqual(c.regulatory || [], ["TILA / Reg Z 12 CFR 1026 (cost-of-credit disclosure)","ECOA / Reg B 12 CFR 1002 (fair lending, no prohibited-basis fields)","UK FCA CONC 4 (pre-contract disclosure)"], 'regulatory anchors intact');
