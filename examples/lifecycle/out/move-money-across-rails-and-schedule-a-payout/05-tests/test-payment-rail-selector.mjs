/* test-payment-rail-selector.mjs — contract-conformance smoke test for PaymentRailSelector.
   Dependency-free: run with `node test-payment-rail-selector.mjs` (adjust the import paths to your project). */
...
const id = "PaymentRailSelector";
const c = core.getComponent(id);
assert.ok(!c.error, 'component resolves');

// 1. every declared token resolves in the system
for (const t of ["accent2","text1","text2","border","radius.md","space.4"]) assert.ok(core.tokenKnown(t), 'token resolves: ' + t);
...
assert.deepStrictEqual(c.regulatory || [], ["NACHA Operating Rules (ACH)","ISO 20022 (wire / RTP messaging)","Reg E 12 CFR 1005 (consumer EFT)"], 'regulatory anchors intact');
