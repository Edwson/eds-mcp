/* test-reconciliation-match.mjs — contract-conformance smoke test for ReconciliationMatch.
   Dependency-free: run with `node test-reconciliation-match.mjs` (adjust the import paths to your project). */
...
const id = "ReconciliationMatch";
const c = core.getComponent(id);
assert.ok(!c.error, 'component resolves');

// 1. every declared token resolves in the system
for (const t of ["green","gold","red","text2","surface2","border","radius.sm","space.3"]) assert.ok(core.tokenKnown(t), 'token resolves: ' + t);
...
assert.deepStrictEqual(c.regulatory || [], ["SOC 1 / ICFR (reconciliation control)","ISO 20022 camt.053 (bank statement)","SEC 17a-4 (audit-trail retention where applicable)"], 'regulatory anchors intact');
