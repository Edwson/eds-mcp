/* test-mandate-consent.mjs — contract-conformance smoke test for MandateConsent.
   Dependency-free: run with `node test-mandate-consent.mjs` (adjust the import paths to your project). */
...
const id = "MandateConsent";
const c = core.getComponent(id);
assert.ok(!c.error, 'component resolves');

// 1. every declared token resolves in the system
for (const t of ["accent2","text1","text2","border","border2","radius.md","space.4"]) assert.ok(core.tokenKnown(t), 'token resolves: ' + t);
...
assert.deepStrictEqual(c.regulatory || [], ["NACHA authorization rules (WEB / PPD / CCD)","SEPA Direct Debit Mandate (Rulebook)","Reg E preauthorized transfers (12 CFR 1005.10)"], 'regulatory anchors intact');
