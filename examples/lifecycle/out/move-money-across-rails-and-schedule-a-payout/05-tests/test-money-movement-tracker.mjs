/* test-money-movement-tracker.mjs — contract-conformance smoke test for MoneyMovementTracker.
   Dependency-free: run with `node test-money-movement-tracker.mjs` (adjust the import paths to your project). */
...
const id = "MoneyMovementTracker";
const c = core.getComponent(id);
assert.ok(!c.error, 'component resolves');

// 1. every declared token resolves in the system
for (const t of ["green","red","gold","text2","border","radius.md","space.3"]) assert.ok(core.tokenKnown(t), 'token resolves: ' + t);
...
assert.deepStrictEqual(c.regulatory || [], ["NACHA return codes (R01–R85)","Reg E error resolution (12 CFR 1005.11)","ISO 20022 pacs.002 (payment status report)"], 'regulatory anchors intact');
