/* test-kyc-stepper.mjs — contract-conformance smoke test for KycStepper.
   Dependency-free: run with `node test-kyc-stepper.mjs` (adjust the import paths to your project). */
...
for (const t of ["accent2","surface","border","radius.md"]) assert.ok(core.tokenKnown(t), 'token resolves: ' + t);
// 2. declared states are a subset of the canonical states
for (const s of ["empty","error"]) assert.ok(core.CANON_STATES.includes(s), 'canonical state: ' + s);
// 3. regulated component carries its anchor(s)
assert.deepStrictEqual(c.regulatory || [], ["FinCEN CDD 31 CFR 1010.230","FATF Rec 10","ASIC AML/CTF"], 'regulatory anchors intact');
