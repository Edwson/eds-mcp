/* test-audit-trail.mjs — contract-conformance smoke test for AuditTrail.
   Dependency-free: run with `node test-audit-trail.mjs` (adjust the import paths to your project). */
...
for (const t of ["surface","border","text2","type.mono","green"]) assert.ok(core.tokenKnown(t), 'token resolves: ' + t);
...
assert.deepStrictEqual(c.regulatory || [], ["SEC Rule 17a-4(f) (append-only retention)","SR 11-7"], 'regulatory anchors intact');
